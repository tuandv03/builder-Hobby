...
// Lấy inventory từ DB
export const getCardSetByCode: RequestHandler = async (req, res) => {
  const { set_code } = req.query as { set_code?: string };
  if (!set_code) {
    return res.status(400).json({ error: "Thiếu tham số set_code" });
  }
  try {
    const data = await queryDb(
      `SELECT c.card_code, c.card_name, c.set_rarity_code,
         ...
         WHERE c.set_code = $1 AND c.set_rarity LIKE 'Qua%'
